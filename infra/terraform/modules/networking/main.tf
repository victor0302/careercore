# Networking module — Phase 1
# Provisions: VPC, public/private subnets (×2 AZs), Internet Gateway,
# public route table, and security groups for compute and database tiers.
...
# Security group for ECS Fargate tasks (outbound-only from private subnets)
resource "aws_security_group" "compute" {
  name        = "${var.app_name}-${var.environment}-compute-sg"
  description = "ECS Fargate tasks — allow all egress"
  vpc_id      = aws_vpc.main.id
...
# Security group for RDS — allows PostgreSQL from the compute SG only
