...
### 12.42 Issue #104 — Job detail and analysis view (PR #119)
...
When adding new fields to the job detail API response (e.g., raw requirements text), add
them to `JobDetailRead` and `JobAnalysisDetailRead` in `frontend/src/types/index.ts` in
the same PR. The deprecation comment on `JobDescription` is the signal to migrate
remaining callers (`jobs/new/page.tsx`) before the next major refactor.
